...
is.na(Rainfall_BR_processed)

summary(Rainfall_BR_processed$Rainfall)
  #mutate(Rainfall.clean=zoo::na.approx(Rainfall))
is.na(Temp_BR_processed)

summary(Temp_BR_processed$Temperature_C)
#temp_processed_cleaned <- Temp_BR_processed %>% 
  #mutate(Temperature_C)

#in the assignment there will be a data set that has missing observations but there are no corresponding NAs with rows. We will need to create a column with the time interval, such as monthly, and then left join that column with the data set. That will create columns with NA values for the missing observations
...
Rain_decomposed <- stl(Rainfall_ts, s.window ="periodic")
plot(Rain_decomposed)

#the remainder is what is left after subtracting the seasonal and trend component. It is the variation in the dataset that is not explained by the trend and the seasonality. There remainder should be random and not have a pattern. The remainder values should also be a mix of positive and negative values.
