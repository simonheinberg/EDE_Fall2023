...
sites$dow <- wday(sites$date) #note: wday collects the day of the week and lists it as a numeric value

## What class is the dow column?
class(sites$dow)

#List the unique values in the column
unique(sites$dow) #note: here we see that the only days of the week in this dataset are 4, 5, and 3

#Summary - is it meaningful?
summary(sites$dow)

#note: factors are grouping variables
```
