...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + #why do we need to say that shape=as.factor instead of just saying 'year'
  #if we don't use as.factor then it will list 'year' as a continuous variable
  geom_point()
print(PMplot)
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #what is the nrow=3 doing: assigning the plots to show up in 3 rows
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #how is facet_grid different than facet_wrap: use facet_grid when foltering dataset by 2 variables and facet_wrap for 1 variable
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = "lm") + #lm stands for linear model, ensuring that it's a straight line 
...
Nutrientplot6 <-
  ...
  geom_freqpoly(aes(x = tn_ug), color = "darkred") + #here we have color outside the aes() function, allowing us to manually assign a color to the plot. If using color inside the aes() function, then you can assign color based on the values found in a given column. It will choose default colors unless you assign other colors through a theme
