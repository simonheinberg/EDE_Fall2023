...
#Load data
nutrient_data <- read.csv(here("Data/Processed_KEY/NTL-LTER_Lake_Nutrients_PeterPaul_Processed.csv"),
...
#Construct the plot, echo=false means it won't show and eval=false means it won't run
nutrient_data %>% 
...
plot_it <- function(the_paramter,start_year,end_year){
    #Construct the plot
  nutrient_data %>% 
    filter(year4 > 1990 & year4 < 2005) %>% 
    ggplot(
...
selectInput(
  ...
  choices=c('tp_ug','tn_ug','no23','p04'),
  selected='tp_ug'
)
