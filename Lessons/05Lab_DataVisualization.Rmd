...
#Create a plot
Plot_text <- 
ggplot(data=data.frame(x=0,y=0))+ #we're creating a data frame of one point and then assigning x and y values of 0
  geom_point(aes(x=x,y=y)) +
  geom_text(
    x=0, 
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6')) #why are there commas between the text here: to read in part of this line as code and part of the line as text
    )
...
ggplot (PeterPaul.chem.nutrients,
  ...
  geombar()+ #I'm getting an error that it can't find geombar
...
surface_temp_1999_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0 & year4 == 1999) %>% #what is filter doing here: it's selecting rows that satisfy both conditions, depth and year4 are each columns and 0 and 1999 are values in the respective columns
  ggplot(
    mapping = aes(
      x=sampledate,
      y=temperature_C,
...
my_theme <- theme_base() + 
  theme(
    line = element_line( #what is the line element in a theme: this is assigning a color to any line you plot with the function geom_line, it won't apply to other types of plots
      color='red',
      linewidth =2
    ),
...
surface.temp<-ggplot(filter(PeterPaul.chem.nutrients, depth==0),
                     aes(y=temperature_C,
                         x=daynum,
                         color=year4))+
  facet_wrap(vars(lakename), nrow=2)+ 
  geom_point()+ 
  ylab("Surface Temperature") #what is ylab?
surface.temp
#2. 
...
ggplot(EPAair, aes(
  ...
  geom_point(alpha=.5)+ #alpha changes the transparency of the dot
  geom_smooth(method="lm") #method="lm" indicates that it's a linear trend line, and geom_smooth creates a trend line
