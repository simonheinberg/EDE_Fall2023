---
title: 'SimonHeinberg_A06 - Crafting Reports'
...
```{r echo=FALSE, fig.align='left', fig.cap="Ozone Levels in 2018", fig.height=4}

boxplot_2018 <- epa_data %>% 
  filter(Year==2018) %>% 
  drop_na(Ozone) %>%
...
```{r echo=FALSE, fig.align='left', fig.cap="Ozone Levels in 2019", fig.height=4}
boxplot_2019 <- epa_data %>% 
  filter(Year==2019) %>% 
  drop_na(Ozone) %>%
  ggplot(aes(x=factor(Month,
                      levels=1:12,
...
[My Git Repository](https://github.com/simonheinberg/EDE_Fall2023.git)